...
- Decision: treat `Nano Banana 2` `Images & Text` runs as one mixed-output job that produces one generated image asset plus `Smart Output` text descriptors, and hydrate completed outputs by actual persisted output kind instead of only by `nodeRunPayload.outputType`.
